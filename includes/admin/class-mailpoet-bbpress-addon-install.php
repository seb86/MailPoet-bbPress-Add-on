 ...
 * @author 		Sebs Studio
 * @category 	Admin
 * @package 	MailPoet bbPress Add-on/Admin/Classes
 * @version 	1.0.0
 */

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

	if ( ! class_exists( 'MailPoet_bbPress_Addon_Install' ) ) {

	/**
	 * MailPoet_bbPress_Addon_Install Class
	 */
	class MailPoet_bbPress_Addon_Install {

		/**
		 * Hook in tabs.
		 */
		public function __construct() {
			register_activation_hook( MAILPOET_BBPRESS_ADDON_FILE, array( &$this, 'install' ) );
		...
		public function check_version() {
			if ( ! defined( 'IFRAME_REQUEST' ) && ( get_option( 'mailpoet_bbpress_addon_version' ) != MailPoet_bbPress_Addon()->version || get_option( 'mailpoet_bbpress_addon_db_version' ) != MailPoet_bbPress_Addon()->version ) ) {
				$this->install();
			}
		}
		...
		public function install_actions() {
			// Update button
			if ( ! empty( $_GET['do_update_mailpoet_bbpress_addon'] ) ) {
		...
		function in_plugin_update_message() {
			$response = wp_remote_get( MAILPOET_BBPRESS_ADDON_README_FILE );
			...
			if ( ! is_wp_error( $response ) && ! empty( $response['body'] ) ) {
				...
				if ( preg_match( $regexp, $response['body'], $matches ) ) {
					...
					echo ' ' . __( 'What\'s new:', 'mailpoet_bbpress_addon' ) . '<div style="font-weight: normal;">';
...
return new MailPoet_bbPress_Addon_Install();
